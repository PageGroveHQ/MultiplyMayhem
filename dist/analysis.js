import {factKey} from './math-games.js';
export function analyzeRun(run){
 const facts=new Map();for(const answer of run.answers)if(!answer.retry)facts.set(factKey({...answer,operation:answer.operation||run.operation}),answer);
...
export function trialKey(run){return JSON.stringify([run.operation||'multiply',run.variant||'tables',run.timeLimitMs||300000,run.totalFacts||null,run.questionSet||((run.operation||'multiply')==='multiply'&&run.totalFacts===(run.tables||[]).length*10?(run.tables||[]).flatMap(a=>Array.from({length:10},(_,b)=>a+','+b)).sort().join(';'):null),run.mode||'parent',run.mode==='solo'?run.format:null,[...new Set(run.tables)].sort((a,b)=>a-b),!!run.smart]);}
export function isFinishedTrial(run){return run.timed&&run.trialVersion===2&&run.endReason==='completed'&&Number.isFinite(run.elapsedMs)&&run.elapsedMs>=0&&run.elapsedMs<=(run.timeLimitMs||300000)&&analyzeRun(run).remaining===0;}

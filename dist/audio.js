export class GameAudio{
 constructor(storage,{contextFactory=()=>new (globalThis.AudioContext||globalThis.webkitAudioContext)(),fetcher=url=>fetch(url)}={}){
  this.storage=storage;this.contextFactory=contextFactory;this.fetcher=fetcher;this.prefs={music:false,effects:true};this.buffers=new Map();this.sources=new Set();this.active=false;this.generation=0;this.lastTick=null;
 ...
 async start(){this.stop();this.active=true;const generation=this.generation;if(await this.unlock()){if(generation!==this.generation||!this.active)return;this.play('start');this.play('starlight-loop',true);}}
 finish(expired){this.stop();this.play(expired?'finish':'clear');}
 ...
 set(key,value){if(!['music','effects'].includes(key))return;this.prefs[key]=!!value;try{this.storage.setItem('star-quest.audio.v1',JSON.stringify(this.prefs));}catch{}
  ...
  if(value)this.unlock().then(ok=>{if(ok){if(key==='music')this.play('starlight-loop',true);else this.play('correct');}});
 }

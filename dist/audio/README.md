# Star Quest audio

Original synthesized composition and effects created for this game. No third-party music, recordings, samples, or recognizable game melodies are used. These project-generated audio assets may be used, modified, and redistributed freely, including with this game; no attribution or payment is required.

- starlight-loop.wav: 32-second looping space chiptune, 120 BPM.
- start.wav: mission launch arpeggio.
- correct.wav: correct-answer cue.
- learn.wav: gentle practice cue.
- tick.wav: one tick, played once per remaining second from 10 through 1.
- finish.wav: time expired.
- clear.wav: completed-set fanfare.

Generated reproducibly by scripts/compose-audio.mjs. Mono 22,050 Hz, 16-bit PCM WAV. Music and effects have independent controls. Music defaults off, effects on; audio begins only following an interaction. Hidden tabs suspend audio, while the trial clock keeps running.


...
export function parseAnswer(raw){const text=String(raw).trim();return /^\d{1,2}$/.test(text)&&Number(text)<=81?Number(text):null;}
...
export function runLabel(run){return `${run.mode==='solo'?'Solo · '+({typing:'Typing',choice:'Multiple choice',mixed:'Mixed'}[run.format]||'Typing'):'Parent-led'}${run.timed?' · 5-minute challenge':''}`;}

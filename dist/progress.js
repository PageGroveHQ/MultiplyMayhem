...
BADGES.push(...['columns','blocks'].map(format=>({id:'solo-'+format,icon:format==='columns'?'▤':'▦',title:format==='columns'?'Column captain':'Block builder',description:'Finish a mission using '+(format==='columns'?'interactive columns':'place-value blocks')+'.',goal:1,value:(s,r)=>r.filter(x=>x.mode==='solo'&&x.format===format&&x.answers.length).length})),{id:'blitz-perfect',icon:'ϟ',title:'Lightning learner',description:'Finish an addition blitz set with every answer correct.',goal:1,value:(s,r)=>r.filter(x=>x.operation==='add'&&['blitz','carry'].includes(x.variant)&&x.endReason==='completed'&&x.answers.length&&x.answers.every(a=>a.ok)).length});
